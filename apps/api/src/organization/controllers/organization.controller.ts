...
export class OrganizationController {
  ...
  @Get('/by-sandbox-auth-token/:authToken')
  @ApiOperation({
    summary: 'Get organization by sandbox auth token',
    operationId: 'getOrganizationBySandboxAuthToken',
  })
  @ApiResponse({
    status: 200,
    description: 'Organization',
  ...
  async getBySandboxAuthToken(@Param('authToken') authToken: string): Promise<OrganizationDto> {
    const organization = await this.organizationService.findBySandboxAuthToken(authToken)
    if (!organization) {
      throw new NotFoundException(`Organization with sandbox auth token ${authToken} not found`)
    }
